...
struct bezier {
    ...
    vec2 operator() (vec2 s) const {
        vec2 valRen = vec2(pow(1 - s, 3) * (*this).coeff(0).x + 3 * pow(1 - s, 2) * s * (*this).coeff(1).x + 3 * (1 - s) * pow(s, 2) * (*this).coeff(2).x + pow(s, 3) * (*this).coeff(3).x, pow(1 - s, 3) * (*this).coeff(0).y + 3 * pow(1 - s, 2) * s * (*this).coeff(1).y + 3 * (1 - s) * pow(s, 2) * (*this).coeff(2).y + pow(s, 3) * (*this).coeff(3).y);
        return valRen;
    }
